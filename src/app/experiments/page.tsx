...
const experiments = [
  {
    ...
    hypothesis:
      "JSON-LDを配置したページへのAIクローラーの月次アクセス数は、JSON-LDなしページより多い",
    pages: [
  ...
  },
  {
    ...
    startDate: "2026-02-22",
    hypothesis:
      "独自造語を定義したページがAIクローラーにクロールされた後、生成AIへの質問で定義が返答される",
    pages: [
      {
        label: "造語定義ページ（JSON-LDあり）",
        href: "/terms/geo-shock-index-a",
      },
    ],
    metric: "初回反映までの日数、定義一致度（Δm）",
    result:
      "観測日: 2026年2月22日　結果: Perplexityが造語「GEOショック指数」の定義を正確に回答。　反映速度: 公開から約1日",
  },
  {
    id: "EXP-003",
  ...
  },
  {
    ...
    startDate: "2026-02-22",
    hypothesis:
      "GEO特化型記事の方が感情型デコイ記事よりAIクローラーの訪問頻度が高い",
    pages: [
      { label: "GEO特化型記事", href: "/articles/what-is-geo" },
      {
        label: "人間向けデコイ記事",
        href: "/articles/ai-and-content-creators",
      },
    ],
    metric: "月次クロール回数の差",
    result: "観測中",
  },
  {
    ...
    startDate: "2026-02-22",
    hypothesis:
      "GEOショックという概念がJSON-LDで定義されたページがクロールされた後、生成AIへの質問で定義が返答される",
    pages: [
  ...
  {
    id: "EXP-006",
    title: "LDO仮説検証：llms.txt限定実装とAIクローラー挙動の観測",
    status: "観測中",
    ...
    metric:
      "llms.txtアクセス有無、LDOページのクロール頻度変化、「LDO」造語のAI反映速度（日数）",
    result: "観測中 — データ蓄積待ち",
  },
];
          ...
          <div key={exp.id} className="card" style={{ padding: "1.5rem" }}>
            ...
            >
              <div>
                ...
                <p style={{ color: "var(--neutral-800)", lineHeight: 1.6 }}>
                  {exp.hypothesis}
                </p>
              </div>
              <div>
