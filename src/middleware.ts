...
const MALICIOUS_RULES: { pattern: RegExp; category: string }[] = [
  ...
  {
    pattern: /\/config\.(php|yml|yaml|json|ini)/i,
    category: "Scan-ConfigLeak",
  },
...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error("[GEO Findings] insert error:", error.message);
    }
  } catch (e) {
    console.error("[GEO Findings] middleware error:", e);
  }
